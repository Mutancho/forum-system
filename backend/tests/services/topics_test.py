...
class TestGetAll_Should(TestCase):
    ...
    async def test_get_all(self, mock_read_query):
        # Arrange
...
class TestDelete_Should(TestCase):
    ...
    async def test_delete_success(self, mock_update_query, mock_get_current_user, mock_topic_exists):
    ...
    async def test_delete_not_found(self, mock_topic_exists):
        result = await delete(C.TOKEN, C.TOPIC_ID_1)
    ...
    async def test_delete_bad_request(self, mock_is_admin, mock_get_current_user, mock_topic_exists):
        result = await delete(C.TOKEN, C.TOPIC_ID_1)
    ...
    @patch('services.topic_service.update_query')
    async def test_delete_success_admin(self, mock_update_query, mock_is_admin, mock_get_current_user, mock_topic_exists):
        result = await delete(C.TOKEN, C.TOPIC_ID_1)
        self.assertEqual(result, UpdateStatus.SUCCESS)
        mock_update_query.assert_called_once_with("DELETE FROM topics WHERE id = ?", (C.TOPIC_ID_1,))
...
class TestUpdate_Should(TestCase):
    ...
    async def test_update_success(self, mock_update_query, mock_get_current_user):
    ...
    async def test_update_not_found(self, mock_update_query, mock_get_current_user):
        topic = TopicWithContent(id=None, title=C.TOPIC_TITLE, content=C.TOPIC_CONTENT)
        result = await update(C.TOKEN, C.TOPIC_ID_1, topic)
        self.assertEqual(result, UpdateStatus.NOT_FOUND)
...
class TestLock_Should(TestCase):
    ...
    @patch('services.topic_service.is_admin', return_value=True)
    @patch('services.topic_service.update_query', return_value=1)
    async def test_unlock_success(self, mock_update_query, mock_is_admin):
    ...
    @patch('services.topic_service.is_admin', return_value=True)
    @patch('services.topic_service.update_query', return_value=0)
    async def test_unlock_not_found(self, mock_update_query, mock_is_admin):
        result = await unlock(C.TOKEN, C.TOPIC_ID_1)
        self.assertEqual(result, UpdateStatus.NOT_FOUND)

    @patch('services.topic_service.is_admin', return_value=False)
    async def test_unlock_not_admin(self, mock_is_admin):
        result = await unlock(C.TOKEN, C.TOPIC_ID_1)
        self.assertIsNone(result)
...
class TestBestReply_Should(TestCase):
    ...
    async def test_set_best_reply_not_found(self, mock_exists_by_id, mock_update_query, mock_get_current_user):
    ...
    async def test_set_best_reply_no_permission(self, mock_exists_by_id, mock_update_query, mock_get_current_user):
        result = await set_best_reply(C.TOKEN, C.TOPIC_ID_1, self.topic_best_reply)
        self.assertEqual(result, UpdateStatus.NOT_FOUND)
    ...
    @patch('services.topic_service.update_query', return_value=1)
    @patch('services.topic_service.exists_by_id', return_value=True)
    async def test_set_best_reply_success(self, mock_exists_by_id, mock_update_query, mock_get_current_user):
        result = await set_best_reply(C.TOKEN, C.TOPIC_ID_1, self.topic_best_reply)
        self.assertEqual(result, UpdateStatus.SUCCESS)
