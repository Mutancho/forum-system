...
class ReplyService_Should(unittest.TestCase):
    ...
    async def test_getById_returnsEmptyDictionary_when_dataIsNotPresent(self, mock_get_votes_for_reply):
    ...
    @patch('services.reply_service.get_votes_for_reply', autospec=True)
    async def test_getById_returnsDictionaryFromReplyWithVotes_when_dataIsPresent(self, mock_get_votes_for_reply):
        with patch('services.reply_service.read_query') as mock_read_query:
    ...
    async def test_create_returnDictionary(self, mock_insert_query):
    ...
    async def test_updateReply_returnDictionaryWithUpdatedReply(self, mock_update_query):
        with patch('services.reply_service.read_query') as mock_read_query:
    ...
    async def test_replyVote_returnVote(self):
    ...
    async def test_updateReplyVote_returnVote(self):
    ...
    async def test_delete_returnVote(self):
        reply_service.update_query = Mock()

        result = await reply_service.delete(1)
        reply_service.update_query.assert_called_once()

        reply_service.update_query.reset_mock()

    @patch('services.reply_service.read_query', autospec=True)
    async def test_alreadyVoted_returnTrue_when_dataIsPresent(self, mock_read_query):
    ...
    @patch('services.reply_service.read_query', autospec=True)
    async def test_alreadyVoted_returnFalse_when_dataIsNotPresent(self, mock_read_query):
        mock_read_query.return_value = []
        reply_service.oauth2.get_current_user = lambda token: 1
    ...
    async def test_getVotesForReply_returnDictionaryWithZeros_when_dataIsNotPresent(self):
        with patch('services.reply_service.read_query') as mock_read_query:
            mock_read_query.return_value = []
    ...
    async def test_getVotesForReply_returnDictionaryCorrectCount_when_dataIsPresent(self):  #
        with patch('services.reply_service.read_query') as mock_read_query:
    ...
    @patch('services.reply_service.read_query', autospec=True)
    async def test_isReplyOwner_returnFalse_when_dataIdIsNotAuthUserId(self, mock_read_query):
    ...
    @patch('services.reply_service.read_query', autospec=True)
    async def test_isReplyOwner_returnTrue_when_dataIdIsAuthUserId(self, mock_read_query):
    ...
    @patch('services.reply_service.read_query', autospec=True)
    async def test_existsById_returnFalse_when_dataIsNotPresent(self, mock_read_query):
        mock_read_query.return_value = []
        reply_service.oauth2.get_current_user = lambda token: 1
    ...
    @patch('services.reply_service.read_query', autospec=True)
    async def test_existsById_returnTrue_when_dataIsPresent(self, mock_read_query):
        mock_read_query.return_value = [(1,)]
        reply_service.oauth2.get_current_user = lambda token: 1
