...
class UserService_Should(unittest.TestCase):
    ...
    def test_create_returnsCreatedMessage(self, mock_insert_query):
        async def async_test():
            test_message = Message(id=None, content='I said testt', created_at=None, sender_id=1, receiver_id=2)
            with patch('services.message_service.read_query') as mock_read_query:
                mock_insert_query.return_value = 1
                mock_read_query.return_value = [('2023-04-14',)]

                result = await message_service.create(test_message)
    ...
    def test_getAllMyConversations_returnsListOfDisplayUser(self, mock_read_query):
        async def async_test():
            ...
            mock_read_query.side_effect = [[(1,), (1,)], [(2,), (2,)], [(1, 'username1', 'email@us1.com', 'password1',
                                                                         'user',
                                                                         datetime.datetime.strptime(
                                                                             '04/14/2023, 13:58:43',
                                                                             "%m/%d/%Y, %H:%M:%S")),
                                                                        (2, 'username2', 'email@us2.com', 'password2',
                                                                         'user',
                                                                         datetime.datetime.strptime(
                                                                             '04/14/2023, 14:30:43',
                                                                             "%m/%d/%Y, %H:%M:%S"))]]

            result = await message_service.get_all_my_conversations('token')
    ...
    def test_getAllMyConversationsWith_returnsDictOfMessages(self, mock_read_query):
        async def async_test():
            ...
            mock_read_query.return_value = [
                (1, 'my message', datetime.datetime.strptime('04/14/2023, 13:58:43', "%m/%d/%Y, %H:%M:%S"), 2),
            ...
            result = await message_service.get_all_my_conversations_with('token', 2)
