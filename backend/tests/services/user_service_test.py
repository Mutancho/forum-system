...
class UserService_Should(unittest.TestCase):
    ...
    async def test_all_createsListOfUsers_when_dataIsPresent(self, mock_read_query):
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_all_returnsEmptyListOfUsers_when_dataIsNotPresent(self, mock_read_query):
        mock_read_query.return_value = []
        result = await list(user_service.all())
        self.assertEqual([], result)

    @patch('services.user_service.insert_query', autospec=True)
    async def test_create_createsUser(self, mock_insert_query):
    ...
    async def test_update_returnsUpdateUser(self):
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_checkUniqueUpdateEmailPassword_returnsFalse_when_dataIsNotPresent(self, mock_read_query):
        user = UpdateUser(username='Testing', email='test@gmasil.com', password='123456')
        mock_read_query.return_value = []
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_checkUniqueUpdateEmailPassword_returnsTrue_when_dataIsPresent(self, mock_read_query):
        user = UpdateUser(username='Testing', email='test@gmasil.com', password='123456')
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_getById_returnsUser_when_dataIsPresent(self, mock_read_query):
        mock_read_query.return_value = [(1, 'Test_username', 'Test_email@test.ts', 'Test_role', '2023-04-14')]
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_existsByUsernameEmail_returnsTrue_when_dataIsPresent(self, mock_read_query):
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_existsByUsernameEmail_returnsFalse_when_dataIsNotPresent(self, mock_read_query):
        user = User(id=1, username='Test_username', email='Test_email@test.ts', password='*********', role='Test_role',
                    created_at='2023-04-14')
        mock_read_query.return_value = []
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_existsById_returnsTrue_when_dataIsPresent(self, mock_read_query):
        user = User(id=1, username='Test_username', email='Test_email@test.ts', password='*********', role='Test_role',
                    created_at='2023-04-14')
        mock_read_query.return_value = [(user,)]
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_existsById_returnsFalse_when_dataIsNotPresent(self, mock_read_query):
        mock_read_query.return_value = []
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_login_returnsToken_when_EmailLogin(self, mock_read_query):
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_login_returnsToken_when_UsernameLogin(self, mock_read_query):
    ...
    async def test_giveAccess_when_userHasPermissionsForCategory(self):
    ...
    async def test_giveAccess_when_userHasNoPermissionsForCategory(self):
        with patch('services.user_service.user_has_permissions_for_category') as mock_user_has_permissions_for_category:
            member_access = Member(user_id=1, category_id=1, read_access=True, write_access=True)
    ...
    async def test_revokeAccess(self):
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_userHasPermissionsForCategory_returnsTrue_when_dataIsPresent(self, mock_read_query):
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_userHasPermissionsForCategory_returnsFalse_when_dataIsNotPresent(self, mock_read_query):
        mock_read_query.return_value = []
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_verifyCredentials_returnsTrue_when_EmailLogin_exists(self, mock_read_query):
        email_login = EmailLogin(email='Test_email@test.ts', password='*********')
        mock_read_query.return_value = [(1, 'Test_username', 'Test_email@test.ts')]
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_verifyCredentials_returnsTrue_when_UsernameLogin_exists(self, mock_read_query):
        username_login = UsernameLogin(username='Test_username', password='*********')
        mock_read_query.return_value = [(1, 'Test_username', 'Test_email@test.ts')]
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_verifyCredentials_returnsFalse_when_EmailLogin_exists(self, mock_read_query):
        email_login = EmailLogin(email='Test_email@test.ts', password='*********')
        mock_read_query.return_value = []
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_verifyCredentials_returnsFalse_when_UsernameLogin_exists(self, mock_read_query):
        username_login = UsernameLogin(username='Test_username', password='*********')
        mock_read_query.return_value = []
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_isUserAuthorizedToGetDelete_returnsTrue_when_userIsHimself(self, mock_read_query):
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_isUserAuthorizedToGetDelete_returnsTrue_when_userIsAdmin(self, mock_read_query):
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_isUserAuthorizedToGetDelete_returnsFalse_when_userIsNotAdminOrHimself(self, mock_read_query):
        mock_read_query.return_value = [('not admin',)]
        user_service.oauth2.get_current_user = lambda token: 0
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_isAdmin_returnsTrue_when_userIsAdmin(self, mock_read_query):
        mock_read_query.return_value = [('admin',)]
        user_service.oauth2.get_current_user = lambda token: 1
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_isAdmin_returnsFalse_when_userIsNotAdmin(self, mock_read_query):
        mock_read_query.return_value = [('not admin',)]
        user_service.oauth2.get_current_user = lambda token: 1
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_verifyPassword_returnsTrue_when_EmailLogin_passwordIsEqualToDecoded(self, mock_read_query):
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_verifyPassword_returnsTrue_when_UsernameLogin_passwordIsEqualToDecoded(self, mock_read_query):
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_verifyPassword_returnsFalse_when_EmailLogin_passwordIsNotEqualToDecoded(self, mock_read_query):
        email_login = EmailLogin(email='Test_email@test.ts', password='123456')
    ...
    @patch('services.user_service.read_query', autospec=True)
    async def test_verifyPassword_returnsFalse_when_UsernameLogin_passwordIsNotEqualToDecoded(self, mock_read_query):
        username_login = UsernameLogin(username='Test_username', password='123456')
        mock_read_query.return_value = [('notHashFor:123456',)]
