...
class ReplyService_Should(unittest.TestCase):
    ...
    def test_getById_returnsEmptyDictionary_when_dataIsNotPresent(self,mock_read_query):
        mock_read_query.return_value = []
        reply_service.get_votes_for_reply = lambda id: {'upvote': 0,'downvote': 0}

        result = reply_service.get_by_id(1)
        self.assertEqual({},result)

    @patch('services.reply_service.read_query', autospec=True)
    def test_getById_returnsDictionaryFromReplyWithVotes_when_dataIsPresent(self, mock_read_query):
    ...
    @patch('services.reply_service.update_query', autospec=True)
    def test_updateReply_returnDictionaryWithUpdatedReply(self, mock_update_query):
        mock_update_query.return_value = None
        with patch('services.reply_service.update_query') as mock_read_query:
            mock_read_query.return_value =[(datetime.strptime('04/14/2023, 13:58:43',"%m/%d/%Y, %H:%M:%S"),)]
            update_reply = UpdateReply(content='new content')
            result = reply_service.update_reply(1,update_reply)

        expected ={'content': 'new content',
                    'downvote': 0,
                    'id': 1,
                    'update on': '21-04-2023 13:49:37',
                    'upvote': 0}
        self.assertEqual(expected,result)
