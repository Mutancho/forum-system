...
from schemas.topic import TopicWithUserAndCategory, BaseTopic, TopicBestReply
from routers.helper_functions import http_validations, Constants as C
...
@router_topic.post("/")
def create_topic(category_id: int, topic: TopicWithUserAndCategory, auth: str = Header(alias="Authorization")):
    token = auth[8:-1]
...
def topic_update(topic_id: int, topic: BaseTopic, auth: str = Header(alias="Authorization")):
    token = auth[8:-1]
